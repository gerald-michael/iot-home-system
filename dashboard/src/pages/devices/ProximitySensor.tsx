...
export default function ProximitySensor() {
    ...
    const { data } = useSWR(`${HOST_URL}household/${householdslug}/device/?resourcetype=ProximitySensorReading`, fetcher)
    console.log(data)
    return (
        <Page title="Proximity Sensor | HSSSIOT">
            <Container>
                                    ...
                                    <CardContent>
                                        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
                                            <Typography variant="body2" color="text.secondary">
                                                {fShortenNumber(item.distance)} cm
                                            </Typography>
